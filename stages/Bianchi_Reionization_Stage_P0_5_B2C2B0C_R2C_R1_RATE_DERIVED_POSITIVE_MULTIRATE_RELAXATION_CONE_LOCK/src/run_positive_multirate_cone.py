...
from positive_multirate_cone import (  # noqa: E402
    ...
    two_mode_weight_for_attenuation_inverse,
)
...
def analytic_endpoint_residual(previous, target, equilibrium, attenuation_inverse):
    residuals = {}
    for family in FAMILY_ORDER:
        decay = 1.0 - 1.0 / float(attenuation_inverse[family])
    ...
    for lane in SHAPE_LANES:
        ...
        for interval_index,substep,dt_myr,z_mid in keys:
            ...
            for macro in range(MACROS_PER_CASE):
                ...
                model="NONE"; rates={}; weights={}; attenuation_inverse={}; eq=None; one_traj=None; two_traj=None; refinement={"pass":False,"status":"NOT_RUN","rows":[]}
                if lp['pass']:
                    rates={k:float(v) for k,v in lp['rates_Myr_inv'].items()}
                    attenuation_inverse={k:float(v) for k,v in lp['a'].items()}
                    eq={f:prev[f]+attenuation_inverse[f]*(targ[f]-prev[f]) for f in FAMILY_ORDER}
                    one_traj=trajectory_certificates(prev,eq,rates,dt_myr,model='ONE_MODE')
                    if one_traj['pass']:
                        model='ONE_MODE'
                    else:
                        weights={f:two_mode_weight_for_attenuation_inverse(attenuation_inverse[f],bounds[f][0],bounds[f][1],dt_myr) for f in FAMILY_ORDER}
                ...
                if model!='NONE':
                    ...
                    endpoint_res=analytic_endpoint_residual(prev,targ,eq,attenuation_inverse)
