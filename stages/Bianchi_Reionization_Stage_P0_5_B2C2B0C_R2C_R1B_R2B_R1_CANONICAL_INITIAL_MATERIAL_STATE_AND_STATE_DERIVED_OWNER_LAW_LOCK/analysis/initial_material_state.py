#!/usr/bin/env python3
"""Canonical positive material-state lift at z=6 for R1B-R2B-R1.

The exact global species fractions and temperature are taken from the durable
CANONICAL_DIRECT_REEVOLVED z=6 history row.  The fixed two-scale hierarchy is
used only as a positive spatial shape prior.  H and He nuclei are lifted as
extensive counts in one comoving Mpc^3; the prior thermal shape is multiplied
by one positive global normalization so that the total resolved internal
energy closes exactly.  No per-node fit or clipping is used.
"""
...
def build_initial_material_state(*, r1_root: Path, r2a_root: Path) -> InitialMaterialState:
    ...
    if not (0.0 <= x_hii <= 1.0 and x_hei >= 0.0 and x_heii >= 0.0 and x_heiii >= 0.0):
        raise RuntimeError("invalid canonical species simplex")
    if abs(x_hei + x_heii + x_heiii - 1.0) > 5.0e-15:
        raise RuntimeError("canonical helium simplex does not close")
    ...
    macro = hierarchy.macro_measure(6.0, mapping, macro_template)
    ...
    if raw_total <= 0.0 or target_u <= 0.0:
        raise RuntimeError("non-positive thermal normalization input")
    thermal_factor = target_u / raw_total
    ...
    metadata: dict[str, Any] = {
        ...
        "global_xHeI": x_hei,
        "global_xHeII": x_heii,
        "global_xHeIII": x_heiii,
        "global_U_resolved_erg_cMpc-3": float(target_u),
