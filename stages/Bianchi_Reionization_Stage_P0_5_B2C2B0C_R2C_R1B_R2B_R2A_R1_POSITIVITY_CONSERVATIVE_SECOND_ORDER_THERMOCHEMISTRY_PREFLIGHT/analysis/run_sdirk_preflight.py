#!/usr/bin/env python3
"""Run sealed Attempt 1: MPRK22(1) chemistry + Alexander L-stable SDIRK2 thermal."""
...
ATTEMPT = STAGE / 'attempts/ATTEMPT_1_MPRK22_ALPHA1_LSTABLE_SDIRK2_THERMAL'
LANES = (
...
CANDIDATE_METHOD = 'NONAUTONOMOUS_MPRK22_ALPHA1_PLUS_LSTABLE_ALEXANDER_SDIRK2_THERMAL'
...
def candidate_rows() -> list[dict[str, Any]]:
    trialmod = _load(
        'r2b_r2a_r1_sdirk_preflight_trial',
        STAGE / 'analysis/second_order_sdirk_trial.py',
    )
    rows: list[dict[str, Any]] = []
    for lane in LANES:
        solver = trialmod.SecondOrderSDIRKTrial.from_repo(repo_root=REPO, lane=lane)
        ...
        for partition in PARTITIONS:
            ...
            full = solver.solve(
                state=parent, t0=t0, t1=t1,
                partition=partition, trial_kind='FULL',
            )
            half1 = solver.solve(
                state=parent, t0=t0, t1=mid,
                partition=2 * partition, trial_kind='FIRST_HALF',
            )
            half2 = (
                solver.solve(
                    state=half1.state, t0=mid, t1=t1,
                    partition=2 * partition, trial_kind='SECOND_HALF',
                )
            ...
            rows.append({
                ...
                'passes_local_error': bool(
                    local_error is not None and local_error < LOCAL_ERROR_TOL
                ),
            ...
            if parent_bytes != (parent.values.tobytes(), parent.temperature_K.tobytes()):
                raise RuntimeError('SDIRK candidate trials mutated the parent state')
    return rows
...
def main() -> int:
    ...
    pd.DataFrame(rows).to_csv(ATTEMPT / 'preflight_results.csv', index=False)
    result = {
        'classification': 'R2B_R2A_R1_ATTEMPT_1_SDIRK2_THERMAL_PREFLIGHT',
    ...
    print(json.dumps({
        ...
        'local_errors': {
            f"{row['lane']}:{row['partition']}": row['local_error'] for row in rows
        },
